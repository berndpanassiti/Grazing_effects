...
planthopperDiversity$group = "planthopper"
...
## control
dfPlot$Label[which(dfPlot$diversityType=="TD" & dfPlot$group =="planthopper")] = "TD\n(N=16)"
dfPlot$Label[which(dfPlot$diversityType=="PD" & dfPlot$group =="planthopper")] = "PD\n(N=16)"
dfPlot$Label[which(dfPlot$diversityType=="FD" & dfPlot$group =="planthopper")]= "FD\n(N=16)"
...
#### Inflorescences in mid/late summer
```{r}
dfPlot2 =  dfPlot %>% 
  dplyr::filter(variable %in% c("nInflorescenceCoocc")) %>%
  dplyr::filter(survey_period==2)
...
df_aov_PostHoc <- dfPlot %>%
  dplyr::filter(variable %in% c("nInflorescenceCoocc","height"), survey_period==2)%>%
  dplyr::group_by(variable) %>%
  tidyr::nest() %>%
  dplyr::mutate(.data = .,
                aov_results = data %>% purrr::map(.x = ., .f = ~ TukeyHSD(aov(value ~ grazing_time, data = .))))
...
```{r}
tmp = veg %>% dplyr::select(-treatment,-nSpecies,
                            -coverGrass,-coverHerbs)%>%
...
### Create plot
```{r}
groupLabel2 <- c(expression('early~summer'),
                                  expression('mid/late~summer'))
...
ggplot(aes(y = value, x = Label,fill=grazing_time))+
  ...
  theme(strip.text.y.left = element_text(angle = 0),
        ...
        axis.text = element_text(size =12),
        legend.position="bottom")+
  geom_point(stat = 'summary', fun = 'mean',col="red");plotVeg
```
