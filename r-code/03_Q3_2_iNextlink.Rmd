...
netResults$group = c(rep("Wildbees",3),rep("Butterflies",3),rep("Planthoppers",3))
...
planthopperEstTD$group = "Planthoppers"
planthopperEstPD$group = "Planthoppers"
planthopperEstFD$group = "Planthoppers"
...
netEstimates$group <- factor(netEstimates$group, levels = c("Wildbees", "Butterflies", "Planthoppers"))
netEstimates$type <- factor(netEstimates$type, levels = c("TD", "PD", "FD"))


z = 0
for (i in c("Wildbees","Butterflies","Planthoppers")){
...
vLineDf$group = c(rep("(A) Wildbees",3),rep("(B) Butterflies",3),rep("(C) Planthoppers",3))
...
dfPlot$group <- factor(dfPlot$group, levels = c("Wild bees", "Butterflies", "Planthoppers"))
...
# create plot
baseplot = ggplot(dfPlot, aes(x=SC,ymin=low,ymax=high,fill=treatment))+
  ...
  size=1)+
  scale_linetype_manual(values=c( "dotted","solid")) +
  #geom_vline(data = vLineDf, mapping = aes(xintercept = x_value))+
  ...
  panel.spacing.y = unit(3, "lines"));p
```
